[简体中文](README.md) | [English](README.en.md) | [繁體中文](README.zh-TW.md)

# masterai-top

Open-source poker AI, multiplayer game systems and Chinese metaphysics software by MasterAI. This document preserves the project's existing features, architecture, screenshots and usage notes; the repository contents remain authoritative.

> Validate features, performance, operational history and deployment requirements against the current version. License and third-party asset terms are defined by repository files.


MasterAI develops and curates Texas Hold'em platforms, point-based poker lobbies, tournament systems, CFR poker AI, multiplayer fishing games, and software based on traditional Chinese culture.
Please refer to the code and documentation in the respective repositories for details regarding features, technology stacks, operational requirements, and licensing terms for each project.



MasterAI develops and documents Texas Hold'em platforms, poker tournament systems, CFR poker AI, multiplayer fishing games, and Chinese metaphysics software.


## Featured Projects


| Project | Key Content | Technical Focus |
| --- | --- | --- |
| [Complete Texas Hold'em Solution](https://github.com/masterai-top/TexasHoldem-Poker-Complete-Solution) | Clubs, private rooms, friend games, SNG & MTT workflows | Unity, C++ |
| [Texas Hold'em Tournament Platform](https://github.com/masterai-top/Texas-Holdem-Poker-Tournament-Event-Platform) | Registration, game tables, rankings, and tournament workflows | C++、Tars |
| [Texas Hold'em Point Lobby](https://github.com/masterai-top/Texas-Hold-em-Points-Lobby) | Point/Gold lobbies, clubs, alliances, friend games, SNG, MTT, and operational modules | Unity、C++ |
| [CFR Texas Hold'em AI](https://github.com/masterai-top/cfr-poker-ai-masterai) | CFR strategy training, evaluation, and game theory research | C++ ，python|
| [OceanRaid Fishing Game Platform](https://github.com/masterai-top/OceanRaid-Fishing-Game-Platform) | Client, game services, tournament modes, and operations backend | Cocos/Lua, Python/C++, Node.js |
| [Bazi, Ziwei & Qimen Charting System](https://github.com/masterai-top/Bazi-Ziwei-Qimen-Dunjia-Divination-System-Source-Code) | Bazi, Ziwei Doushu, Qimen Dunjia, and related chart generation workflows | JavaScript |


## Texas Hold'em Platform


The main Texas Hold'em repository documents modules for the Unity client and C++ server, covering product workflows such as clubs, game tables, friend games, SNGs, and MTTs.
Please independently verify dependencies, performance, security, and compliance requirements prior to actual deployment.
<table>
  <tr>
    <td width="50%"><img src="https://raw.githubusercontent.com/masterai-top/TexasHoldem-Poker-Complete-Solution/main/Screenshots/%E5%88%9B%E5%BB%BA%E4%BF%B1%E4%B9%90%E9%83%A8.jpg" alt="德州扑克俱乐部创建界面 Texas Hold'em poker club creation" width="100%"></td>
    <td width="50%"><img src="https://raw.githubusercontent.com/masterai-top/TexasHoldem-Poker-Complete-Solution/main/Screenshots/MTT%E8%B5%9B%E4%BA%8B.jpg" alt="德州扑克 MTT 锦标赛界面 Texas Hold'em MTT tournament" width="100%"></td>
  </tr>
</table>
- [View Complete Texas Hold'em Solution](https://github.com/masterai-top/TexasHoldem-Poker-Complete-Solution)
- [Texas Hold'em Points Lobby](https://github.com/masterai-top/Texas-Hold-em-Points-Lobby)
- [View Texas Hold'em Tournament Platform](https://github.com/masterai-top/Texas-Holdem-Poker-Tournament-Event-Platform)
- [View CFR Poker AI](https://github.com/masterai-top/cfr-poker-ai-masterai)


## Texas Hold'em Points Lobby

The points-lobby project documents coin and points lobbies, poker clubs, leagues, friend tables, SNG, MTT, Short Deck and multiplayer table workflows. The following images come from its current public README.

<table>
  <tr>
    <td width="50%"><img src="https://github.com/user-attachments/assets/7da76d6b-c7fd-4e36-87e8-61f5bd504438" alt="Texas Hold'em points lobby and poker club interface" width="100%"></td>
    <td width="50%"><img src="https://github.com/user-attachments/assets/3dda8ea6-b7d1-4679-8ca1-8469e68b6c97" alt="Texas Hold'em nine-player poker table" width="100%"></td>
  </tr>
</table>

- [View Texas Hold'em Points Lobby](https://github.com/masterai-top/Texas-Hold-em-Points-Lobby)

## CFR Poker AI

The MasterAI CFR project focuses on heads-up no-limit Texas Hold'em strategy research, including CFR, counterfactual value estimation, self-play, training and evaluation components. Performance and win-rate claims require reproducible evaluation against the current version.

<table>
  <tr>
    <td width="50%"><img src="https://github.com/user-attachments/assets/66851632-7b29-4fc3-a35c-76dee4e5930d" alt="MasterAI Texas Hold'em poker AI interface" width="100%"></td>
    <td width="50%"><img src="https://github.com/user-attachments/assets/8cba978a-8a2f-4310-b650-d96adf1dd633" alt="Texas Hold'em AI strategy visualization" width="100%"></td>
  </tr>
</table>

- [View MasterAI CFR Poker AI](https://github.com/masterai-top/cfr-poker-ai-masterai)

## Arcade Fishing Platform


The OceanRaid project documentation covers the Cocos/Lua client, Python/C++ server, Node.js operational APIs, and pages related to fishing tournaments.
Images are sourced from the project's current public screenshot directory.
<table>
  <tr>
    <td width="50%"><img src="https://raw.githubusercontent.com/masterai-top/OceanRaid-Fishing-Game-Platform/main/docs/assets/screenshots/lobby.png" alt="OceanRaid 多人捕鱼游戏大厅 multiplayer fishing game lobby" width="100%"></td>
    <td width="50%"><img src="https://raw.githubusercontent.com/masterai-top/OceanRaid-Fishing-Game-Platform/main/docs/assets/screenshots/tournament-mode.png" alt="OceanRaid 多人捕鱼比赛模式 arcade fishing tournament" width="100%"></td>
  </tr>
</table>
- [View OceanRaid Fishing Game Platform](https://github.com/masterai-top/OceanRaid-Fishing-Game-Platform)


## I Ching and Chinese Metaphysics Software


This comprehensive metaphysics repository covers *Ba Zi* (Four Pillars of Destiny), *Zi Wei Dou Shu* (Purple Star Astrology), *Qi Men Dun Jia*, and related chart generation. The content is intended for software development and the study of traditional metaphysical algorithms;
it should not be used as a substitute for professional medical, legal, financial, or other expert advice.
<table>
  <tr>
    <td width="50%"><img src="https://raw.githubusercontent.com/masterai-top/Bazi-Ziwei-Qimen-Dunjia-Divination-System-Source-Code/main/Screenshots/baizhipaipan.png" alt="八字排盘软件界面 Bazi Four Pillars chart" width="100%"></td>
    <td width="50%"><img src="https://raw.githubusercontent.com/masterai-top/Bazi-Ziwei-Qimen-Dunjia-Divination-System-Source-Code/main/Screenshots/wuxing.png" alt="五行分析软件界面 Five Elements analysis" width="100%"></td>
  </tr>
</table>
- [View Comprehensive Numerology/Divination Charting System](https://github.com/masterai-top/Bazi-Ziwei-Qimen-Dunjia-Divination-System-Source-Code)
- [View Browser-based I Ching Charting Project](https://github.com/masterai-top/Zhouyi-Divination-System-Source-Code)


## Downloads and Documentation


- [All Public Repositories](https://github.com/masterai-top?tab=repositories)
- [MasterAI Project Website](https://masterai-top.github.io/masterai-top/)
- Please obtain versioned downloads from the GitHub Releases page of the respective project.

- Before use, please read the README, LICENSE, SECURITY, and known limitations of the respective project.



## License and Responsible Use


Licenses may vary across repositories.
Before use, modification, distribution, or deployment, please check the specific repository's license, third-party dependencies, and asset licensing, and ensure compliance with local laws and platform rules.


Licenses may differ by repository.
Review the applicable license, third-party notices, and local requirements before use, modification, redistribution, or deployment.


Each repository details the scope of open-source code, dependencies, supported platforms, and licensing. Commercial deployment, third-party resources, payment systems, game rules, privacy, legal protections, and regional compliance requirements must be reviewed on a per-project basis.
Public documentation does not replace written commercial agreements or technical acceptance checklists.



Contact Information
Telegram: @xuzongbin001
Email: masterai918@gmail.com
Website: https://masterai-top.github.io/masterai-top/
Core Focus: Texas Hold'em source code, lobby systems, tournament platforms, club management systems, Texas Hold'em AI, fishing game source code, and I Ching software.

